                    ...
                    <form class="needs-validation" novalidate action="campaigns/detail.html">
                        ...
                        <div class="row">
                            <div class="col-12 col-lg-4 mb-3">
                                ...
                                <input :disabled="dataForm.type == 1"
                                    :class="{ 'is-invalid': validateError.type != '' }" v-model="dataForm.cost"
                                    type="number" class="form-control" id="inputCost" name="cost" placeholder=""
                                    value="0">
                                <div class="invalid-feedback">
                                    {{ validateError.type }}
                                </div>
...
export default {
    ...
    methods: {
        ...
        saveSession() {
            ...
            let session_id = scop.$route.params.session_id;
            axios.post('http://127.0.0.1:8000/api/session/edit/'+session_id, scop.dataForm, {
        ...
        getSession() {
            let scop = this;
            let id = scop.$route.params.session_id;
            let token = localStorage.getItem('token_login_hht');
            axios.get('http://127.0.0.1:8000/api/session/get-edit/' + id, {
                headers: { "Authorization": token }
            })
                .then(function (response) {
                    let dataSession = response.data.data;
                    scop.dataForm.cost = dataSession.cost;
                    scop.dataForm.description = dataSession.description;
                    scop.dataForm.end = dataSession.end;
                    scop.dataForm.id = dataSession.id;
                    scop.dataForm.place_id = dataSession.place_id;
                    scop.dataForm.start = dataSession.start;
                    scop.dataForm.title = dataSession.title;
                    if(dataSession.type == 'normal') {
                        scop.dataForm.type = 1;
                    }else {
                        scop.dataForm.type = 2;
                    }
                    scop.dataForm.vaccinate = dataSession.vaccinate;
                })
                .catch(function (error) {
